...
tuple<bool, double, double, double> RLR(double sc_th0, double sc_thf, double sc_Kmax) {
    ...
    double temp1 = atan2(C, S);
    double temp2 =
            0.125 * (6 - 4 * pow(sc_Kmax, 2) + 2 * cos(sc_th0 - sc_thf) + 4 * sc_Kmax * (sin(sc_th0) - sin(sc_thf)));
...
scaleToStandard(double x0, double y0, double th0, double xf, double yf, double thf, double Kmax) {
    double dx = xf - x0;
    double dy = yf - y0;
...
pair<int, curve> Dubins::shortest_path() {
    ...
    tie(sc_th0, sc_thf, sc_Kmax, lambda) = scaleToStandard(this->x0, this->y0, this->th0, this->xf, this->yf, this->thf,
                                                           this->kmax);
    ...
    for (int i = 0; i < primitives.size(); i++) {
        ...
        curve tempcur = dubinscurve(x0, y0, th0, s1, s2, s3, ksigns[i][0] * kmax, ksigns[i][1] * kmax,
                                    ksigns[i][2] * kmax);
        cout << "L: " << tempcur.L << " \n s1: " << s1 << " \n s2: " << s2 << " \n s3: " << s3 << endl;
    ...
    if (pidx >= 0) {
        ...
        cur = dubinscurve(x0, y0, th0, s1, s2, s3, ksigns[pidx][0] * kmax, ksigns[pidx][1] * kmax,
                          ksigns[pidx][2] * kmax);
